...
from domain.models import RiskCategory, SignalDirection, SignalStrength, TimeHorizon
...
def test_recompute_single_risk() -> None:
    ...
    response: requests.Response = requests.post(
        url=f"{BASE_URL}/api/risks/", json=risk_data
    )
    ...
    response = requests.post(url=f"{BASE_URL}/api/signals/", json=signal1_data)
    assert response.status_code == 201
    ...
    response = requests.post(url=f"{BASE_URL}/api/signals/", json=signal2_data)
    assert response.status_code == 201
    print(f"Created signal: {response.json()['name']}")

    # Recompute the risk
    response = requests.post(url=f"{BASE_URL}/api/risks/{risk_id}/recompute")
    assert response.status_code == 201
...
def test_recompute_all_risks() -> None:
    ...
    response: requests.Response = requests.post(url=f"{BASE_URL}/api/risks/recompute")
    assert response.status_code == 201
    ...
    for assessment in assessments:
        print(
            f"  - Risk {assessment['risk_id']}: score={assessment['risk_score']:.2f}, "
...
def test_likelihood_modifiers() -> None:
    ...
    for name, direction, strength, expected_likelihood in test_cases:
        ...
        response = requests.post(url=f"{BASE_URL}/api/signals/", json=signal_data)
        assert response.status_code == 201
        ...
        assert abs(assessment["effective_likelihood"] - expected_likelihood) < 0.01
        print(
            f"✓ {name}: {assessment['effective_likelihood']:.2f} (expected {expected_likelihood})"
        )

        # Clean up signal for next test
        requests.delete(url=f"{BASE_URL}/api/signals/{signal_id}")
...
def test_likelihood_clamping() -> None:
    ...
    response: requests.Response = requests.post(
        f"{BASE_URL}/api/risks/", json=risk_data
    )
    risk: dict[str, Any] = response.json()
    risk_id: int = risk["id"]
    ...
    for i in range(3):
        ...
        requests.post(url=f"{BASE_URL}/api/signals/", json=signal_data)
    ...
    response = requests.post(url=f"{BASE_URL}/api/risks/", json=risk_data)
    risk = response.json()
    risk_id = risk["id"]
